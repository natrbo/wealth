  ...
  if(pvdAssets.length){
    ...
    h+=`<div class="card" style="padding:18px">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:4px">
        <div class="ctitle" style="margin-bottom:0">PVD สมทบรายเดือน</div>
        ${pvdTotal>0?`<span class="gold tnum" style="font-size:13px;font-weight:600">${baht(pvdTotal)}/ด.</span>`:''}
      </div>
      <div class="muted" style="font-size:11px;margin-bottom:14px">ใส่ยอดเงินที่หักเข้า PVD แต่ละกองต่อเดือน</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ${pvdAssets.map(a=>`
          <div style="background:var(--ink3);border-radius:12px;padding:14px">
            <div style="font-size:10px;color:${CLASSES[a.asset_class].color};margin-bottom:3px">${CLASSES[a.asset_class].label}</div>
            <div style="font-size:12px;font-weight:600;margin-bottom:10px;line-height:1.3">${a.name}</div>
            <input class="num" type="text" inputmode="numeric" value="${+a.plan||''}"
              placeholder="0"
              oninput="setPlan('${a.id}',this.value)"
              style="width:100%;padding:6px 8px;font-size:16px;font-weight:700;background:var(--ink2)">
