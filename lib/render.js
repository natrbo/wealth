...
const TAB_ICONS = {
  overview: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="9"/><rect x="14" y="3" width="7" height="5"/><rect x="14" y="12" width="7" height="9"/><rect x="3" y="16" width="7" height="5"/></svg>`,
  update: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>`,
  dca: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>`,
  tax: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>`,
  networth: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>`,
...
function render(loading){
  ...
  const tabTitles = {
    overview: "ภาพรวมพอร์ตลงทุน",
    update: "อัปเดตมูลค่าสินทรัพย์",
    dca: "แผนลงทุนรายเดือน (DCA)",
    tax: "วางแผนประหยัดภาษีปี 2569",
    networth: "ความมั่งคั่งสุทธิ (Net Worth)",
    business: "จัดการธุรกิจปล่อยจำนอง"
  };
  
  const tabSubs = {
    overview: "ภาพกว้างและสัดส่วนสินทรัพย์ของคุณในปัจจุบัน",
    update: "บันทึกยอดเงินล่าสุดของแต่ละกองทุน/หลักทรัพย์",
    dca: "ปรับปรุงแผนลงทุน DCA เพื่อคาดการณ์ยอดเงินสิ้นปี",
    tax: "คำนวณและวางแผนลดหย่อนภาษีอย่างมีประสิทธิภาพ",
    networth: "แสดงสินทรัพย์ หนี้สิน และความคุ้มครองทั้งหมดของคุณ",
    business: "ติดตามสมุดบัญชีเงินกู้ลูกหนี้และมูลค่าหุ้นบริษัท"
  };
    ...
    <div class="app-container">
      ...
      <aside class="sidebar">
        <div class="sidebar-brand">
          <div class="eyebrow">UNIFIED PORTFOLIO</div>
          <h1>Wealth Ledger</h1>
        </div>
        
        <!-- Total Value inside sidebar -->
        <div class="sidebar-total">
          <div class="sub">มูลค่ารวมพอร์ต</div>
          <div class="total">${baht(t)}</div>
          <div class="sub" style="margin-top: 4px;">โปรไฟล์ปัจจุบัน: <b>${STATE.profile==="fern"?"เฟิร์น":"นัท"}</b></div>
        </div>

        <!-- Sidebar Navigation -->
        <nav class="sidebar-nav">
          ${[["overview","ภาพรวม"],["update","อัปเดต"],["dca","แผน DCA"],["tax","ภาษี"],["networth","มูลค่าสุทธิ"],["business","ธุรกิจ"]]
            .map(([k,l])=>`
              <button class="tab-btn ${STATE.tab===k?"on":""}" onclick="go('${k}')">
                ${TAB_ICONS[k] || ""}
                <span>${l}</span>
              </button>
            `).join("")}
        ...
        <div class="sidebar-footer">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span class="muted" style="font-size: 11px;">โหมดการใช้งาน</span>
      ...
      <div class="main-content">
        ...
        <header class="main-header">
          <div class="header-title">
            <h2>${tabTitles[STATE.tab] || ""}</h2>
            <div class="header-subtitle muted">${tabSubs[STATE.tab] || ""}</div>
          </div>
          
          <div class="header-controls">
            <div class="profiles-wrapper">
              <span class="muted" style="font-size: 11px; margin-right: 10px;">โปรไฟล์:</span>
              <div class="profiles" id="profilebar">${renderProfiles()}</div>
            </div>
          </div>
        </header>
...
// ── contextual person bar — โชว์ใต้แท็บ คุม profile (แท็บทั่วไป) หรือ nw.view (แท็บมูลค่าสุทธิ) ──
function renderProfiles(){
  if(STATE.tab==="networth"){
    const view=(STATE.nw&&STATE.nw.view)||"household";
    return [["fern","เฟิร์น"],["nut","นัท"],["household","รวม"]]
      .map(([k,l])=>`<button class="pf ${view===k?"on":""}" onclick="nwSetView('${k}')">${l}</button>`).join("");
  }
  return [["fern","เฟิร์น"],["nut","นัท"]]
    .map(([k,l])=>`<button class="pf ${STATE.profile===k?"on":""}" onclick="switchProfile('${k}')">${l}</button>`).join("");
}
function refreshProfiles(){const b=document.getElementById("profilebar"); if(b) b.innerHTML=renderProfiles();}
function renderTab(){
  ...
  if(STATE.assets.length===0 && STATE.tab!=="tax" && STATE.tab!=="networth" && STATE.tab!=="business"){
  ...
  ({overview:tabOverview,update:tabUpdate,dca:tabDCA,tax:tabTax,networth:tabNetWorth,business:tabBusiness}[STATE.tab])(m);
}
...
function tabOverview(m){
  ...
  if(hs.length){
    ...
    growthHtml=`<div class="card" style="margin-bottom:0">
      ...
      <div class="muted" style="font-size:11px;margin-bottom:12px">กด ✕ ใต้แท่งเพื่อลบ snapshot ที่ผิดพลาด</div>
      <div style="display:flex;align-items:flex-end;gap:6px;height:160px">
        ${hs.map(s=>{
          const hpx=20+((s.total-min)/rng)*120;
          return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:3px">
            <span class="muted tnum" style="font-size:9px">${(s.total/1e6).toFixed(2)}M</span>
            <div style="width:100%;max-width:36px;height:${hpx}px;background:linear-gradient(180deg,#E4CF9B,#C8A35B);border-radius:3px 3px 0 0"></div>
            <span class="muted" style="font-size:9px">${s.month}</span>
            <button onclick="deleteSnapshot('${s.month}')" title="ลบ"
              style="background:none;border:1px solid var(--line);color:var(--mute);font-size:9px;
                     border-radius:4px;padding:1px 4px;cursor:pointer;font-family:inherit;line-height:1.4">✕</button>
          </div>`;
        }).join('')}
      </div>
    </div>`;
  ...
  m.innerHTML=`<div class="dash-overview">
    ...
    <div class="ov-right">${cardsHtml}</div>
  </div>`;
}

// เรียงกองตามลำดับหมวดสินทรัพย์ (CK) → หมวดเดียวกันอยู่ติดกัน อ่านง่าย
const _classIdx=k=>{const i=CK.indexOf(k);return i<0?CK.length:i;};
const byClassOrder=(a,b)=>(_classIdx(a.asset_class)-_classIdx(b.asset_class))||(a.name||'').localeCompare(b.name||'','th');
...
function tabUpdate(m){
  ...
  buckets().forEach(b=>{
    const assets=STATE.assets.filter(a=>a.bucket===b).sort(byClassOrder);
...
function tabDCA(m){
  ...
  const planned=nonPvd.filter(a=>+a.plan>0||a._isEditingDca).sort(byClassOrder);
  const notPlanned=nonPvd.filter(a=>!(+a.plan>0)&&!a._isEditingDca).sort(byClassOrder);
  const tp=nonPvd.reduce((s,a)=>s+(+a.plan||0),0);
  const cls={}; nonPvd.forEach(a=>{if(+a.plan>0)cls[a.asset_class]=(cls[a.asset_class]||0)+(+a.plan);});
  ...
  let h=`<div class="card" style="padding:20px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        <button class="btn" onclick="showReportMenu()" title="ส่งออกรายงาน"
          style="background:var(--ink3);color:var(--mute);border:1px solid var(--line);font-size:13px">↗</button>
  ...
  if(pvdAssets.length){
    ...
    const pctOk=Math.abs(pctSum-100)<0.05, pctShow=+pctSum.toFixed(1), pctGap=+Math.abs(100-pctSum).toFixed(1);
      ...
      <div style="display:grid;gap:8px">
        ${pvdAssets.map(a=>`
          <div style="display:grid;grid-template-columns:1fr 64px 100px;gap:8px;align-items:center;
            ...
            <div id="pvd-amt-${a.id}" class="tnum" style="font-size:15px;font-weight:700;color:var(--gold);text-align:right">
              ${baht(+a.plan||0)}
            </div>
          </div>`).join('')}
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-top:14px">
        <span id="pvd-sum" style="font-size:12px;font-weight:600;color:${pctOk?'var(--green)':'var(--red)'}">${pctOk?`รวม ${pctShow}% ✓`:`รวม ${pctShow}% — ${pctSum<100?'ขาดอีก '+pctGap:'เกินมา '+pctGap}% (ต้องรวมเป็น 100)`}</span>
        <button class="btn" id="pvd-save-btn" onclick="savePvd()" ${pctOk?'':'disabled'} style="${pctOk?'':'opacity:.45;cursor:not-allowed'}">บันทึก PVD</button>
      </div>
    </div>`;
  }
...
function tabTax(m){
  ...
  const portRMF = STATE.assets.filter(a=>/rmf/i.test(a.name)||/rmf/i.test(a.bucket)).reduce((s,a)=>s+(+a.value||0),0);
  const portESG = STATE.assets.filter(a=>/esg/i.test(a.name)||a.bucket==="ThaiESG").reduce((s,a)=>s+(+a.value||0),0);
  ...
  if(ao.insurance)col2+=`<div class="alert" style="border-left-color:var(--border);margin:8px 0"><span class="muted" style="font-size:11px">สุขภาพ ≤25k · รวมชีวิต ≤100k · บำนาญเติมช่องว่างได้</span></div>`+taxRow("lifeInsurance","ประกันชีวิต")+taxRow("healthInsurance","ประกันสุขภาพตัวเอง");
  col2+=accHead("funds","3. กองทุนเพื่อการออม",`${baht(r.retSum)} / 500k`);
  if(ao.funds){
    col2+=`<div class="alert" style="border-left-color:var(--border);margin:8px 0"><span class="muted" style="font-size:11px">💡 <b>ตัวเลขลดหย่อน</b> = เฉพาะที่ซื้อใหม่ปี 2569 (bought + DCA เหลือ) · ของสะสมปีก่อนลดหย่อนไปแล้ว ดูมูลค่ารวมที่แท็บพอร์ต · กลุ่มเกษียณ RMF+PVD+SSF+บำนาญ รวม ≤500k</span></div>`;
    
    // RMF — แยกพอร์ตรวม / ซื้อปีนี้ / DCA เหลือ
    col2+=deductBlock("RMF","#5B8DEF",portRMF,"rmfBought",dt._rmfBought,dt._future,dt._rmfMonthly,rmfMax);
    col2+=taxRow("pvd","PVD (เงินสะสมปีนี้)");
    col2+=`<div class="arow"><div style="flex:1"><div style="font-size:14px;color:var(--text-secondary)">SSF <span style="font-size:9px;background:rgba(138,153,181,.15);color:var(--text-secondary);border-radius:4px;padding:1px 7px">🔒 สิ้นสิทธิ์</span></div><div class="muted" style="font-size:10px">ซื้อเพิ่มไม่ได้แล้ว</div></div><span class="muted tnum">${baht(STATE.tax.ssf||0)}</span></div>`;
    col2+=taxRow("pension","ประกันบำนาญ");
    col2+=`<div style="border-top:1px solid var(--border);margin:8px 0"></div>`;
    
    // ThaiESG — แยกพอร์ตรวม / ซื้อปีนี้ / DCA เหลือ
    col2+=deductBlock("ThaiESG","#4DD0C7",portESG,"esgBought",dt._esgBought,dt._future,dt._esgMonthly,esgMax);
    
    col2+=`<div style="background:var(--bg-input);border-radius:12px;padding:14px;margin-top:12px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
        <span style="font-size:11px;color:var(--accent-gold);font-weight:700;letter-spacing:1px">🌡️ ขั้นภาษีปัจจุบัน</span>
  ...
  if(ao.donations){
    col2+=`<div class="alert" style="border-left-color:var(--border);margin:8px 0"><span class="muted" style="font-size:11px">Easy E-Receipt ≤50k · 2x e-Donation ≤10%สุทธิ · บริจาค 1x ≤10% (หลัง eDon)</span></div>`;
  ...
  if(hasDca){
    const rmfProj=dt._rmfBought+rmfMonthly*mLeft;
    const esgProj=dt._esgBought+esgMonthly*mLeft;
    ...
    if(rmfMonthly>0) col3+=deductBlockProj("RMF","#5B8DEF",dt._rmfBought,rmfMonthly);
    if(esgMonthly>0) col3+=deductBlockProj("ThaiESG","#4DD0C7",dt._esgBought,esgMonthly);
    const biProj=bracketInfo(rProj.net, rProj.marginal);
    const bracketChanged=rProj.marginal!==r.marginal;
    col3+=`<div style="border-top:2px solid var(--border);margin-top:10px;padding-top:12px">
      <div class="muted" style="font-size:10px;letter-spacing:1px;margin-bottom:8px">คาดการณ์ภาษีสิ้นปี (ถ้า DCA ครบ)</div>

      ${rmfMonthly>0?`<div style="display:flex;justify-content:space-between;padding:4px 0;border-top:1px solid var(--border)">
        <span class="muted" style="font-size:11px">RMF (YTD + DCA)</span>
        <span class="tnum" style="font-size:11px;color:#5B8DEF">${baht(dt._rmfBought)} + ${baht(rmfMonthly*mLeft)} = <b>${baht(rmfProj)}</b></span>
      </div>`:''}
      ${esgMonthly>0?`<div style="display:flex;justify-content:space-between;padding:4px 0;border-top:1px solid var(--border)">
        <span class="muted" style="font-size:11px">ThaiESG (YTD + DCA)</span>
        <span class="tnum" style="font-size:11px;color:#4DD0C7">${baht(dt._esgBought)} + ${baht(esgMonthly*mLeft)} = <b>${baht(esgProj)}</b></span>
      </div>`:''}
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-top:1px solid var(--border);margin-top:2px">
        <span class="muted" style="font-size:11px">ขั้นภาษี (คาด)</span>
        <div style="display:flex;align-items:center;gap:8px">
          ${bracketChanged?`<span style="font-size:11px;color:var(--text-secondary);text-decoration:line-through">${(r.marginal*100).toFixed(0)}%</span>
          <span style="font-size:11px;color:var(--accent-green)">→</span>`:''}
          <span style="font-size:20px;font-weight:700;color:${bracketChanged?'var(--accent-green)':'var(--accent-gold)'}">${(rProj.marginal*100).toFixed(0)}%</span>
        </div>
      </div>
      ${bracketChanged?`<div style="font-size:11px;padding:4px 8px;border-radius:6px;background:rgba(16,185,129,.12);color:var(--accent-green);margin-bottom:6px">
        ✓ ตกขั้นจาก ${(r.marginal*100).toFixed(0)}% → ${(rProj.marginal*100).toFixed(0)}% ถ้า DCA ครบ
      </div>`:`<div style="font-size:11px;color:var(--text-secondary);margin-bottom:6px">
...
function deductBlock(label,color,portVal,boughtKey,boughtVal,future,monthly,maxLimit){
  const futureAmt=monthly*future, totalD=boughtVal+futureAmt;
  return `<div style="background:var(--bg-input);border-radius:12px;padding:16px;margin:12px 0;border:1px solid var(--border)">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
      <span style="color:${color};font-weight:800;font-size:14px">${label}</span>
      <span style="font-size:10px;background:rgba(59,130,246,.12);color:var(--accent-blue);border-radius:4px;padding:2px 8px;font-weight:700">ปีภาษี 2569</span>
    </div>
    <div style="display:flex;justify-content:space-between;padding:6px 0">
      <span class="muted" style="font-size:12px">มูลค่าพอร์ตรวม</span>
      <span class="tnum muted" style="font-size:13px;font-weight:600">${baht(portVal)}</span>
    </div>
    <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0">
      <span style="font-size:12px">ซื้อแล้วปีนี้ (YTD)</span>
      <input id="tax-input-${boughtKey}" class="num" type="text" inputmode="numeric" value="${boughtVal}" oninput="setTax('${boughtKey}',this.value)" style="width:110px;padding:6px 10px;font-size:13px">
    </div>
    <div style="display:flex;justify-content:flex-end;gap:6px;margin:6px 0">
      <button onclick="setTaxAndReload('${boughtKey}', ${maxLimit})" style="font-size:10px;padding:4px 8px;border-radius:6px;background:rgba(99,102,241,0.12);color:var(--accent-blue);border:none;cursor:pointer;font-weight:700;transition:transform 0.1s" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">ซื้อเต็มสิทธิ์</button>
      <button onclick="addTaxAndReload('${boughtKey}', 50000)" style="font-size:10px;padding:4px 8px;border-radius:6px;background:var(--bg-card);color:var(--text-secondary);border:1px solid var(--border);cursor:pointer;font-weight:700;transition:transform 0.1s" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">+50,000</button>
      <button onclick="addTaxAndReload('${boughtKey}', 100000)" style="font-size:10px;padding:4px 8px;border-radius:6px;background:var(--bg-card);color:var(--text-secondary);border:1px solid var(--border);cursor:pointer;font-weight:700;transition:transform 0.1s" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">+100,000</button>
    </div>
    <div style="display:flex;justify-content:space-between;padding:6px 0">
      <span class="muted" style="font-size:12px">แผน DCA ที่เหลือ</span>
      <span class="tnum muted" style="font-size:13px">${future} เดือน × ${baht(monthly)} = ${baht(futureAmt)}</span>
    </div>
    <div style="display:flex;justify-content:space-between;align-items:center;border-top:1px solid var(--border);margin-top:10px;padding-top:10px">
      <span style="font-size:13px;font-weight:700">รวมลดหย่อนปีนี้</span>
      <span class="gold tnum" style="font-size:18px;font-weight:800">${baht(totalD)}</span>
    </div>
  </div>`;
}

function deductBlockProj(label,color,boughtVal,monthly){
  const future=remainingMonths();
  const futureAmt=monthly*future;
  const totalD=boughtVal+futureAmt;
  return `<div style="background:var(--bg-input);border-radius:12px;padding:12px;margin-bottom:8px;border:1px solid var(--border)">
    ...
    <div style="display:flex;justify-content:space-between;padding:6px 0 0;border-top:1px solid var(--border);margin-top:2px">
