...
function nwToggleForm(section){
  ...
  if(STATE.nw._form&&STATE.nw._form.section===section){ STATE.nw._form=null; }
    ...
    STATE.nw._form={section, mode:"add", name:"", type:"mortgage_lending", owner:"household", loanBookOutstanding:"", monthlyInterestIncome:"", avgInterestRate:"", provisionForDoubtful:""};
...
function nwEditRow(section, id){
  ...
  if (section === "incomeStreams" && row.linkedAssetId) {
    if (row.linkedAssetId.startsWith("r")) {
      ...
      if (bizAsset) {
        resolvedAmt = +bizAsset.monthlyInterestIncome || 0;
  ...
  STATE.nw._form={
    ...
    liquid: !!row.liquid,
    loanBookOutstanding: String(row.loanBookOutstanding !== undefined ? row.loanBookOutstanding : ""),
    monthlyInterestIncome: String(row.monthlyInterestIncome !== undefined ? row.monthlyInterestIncome : ""),
    avgInterestRate: String(row.avgInterestRate !== undefined ? row.avgInterestRate : ""),
    provisionForDoubtful: String(row.provisionForDoubtful !== undefined ? row.provisionForDoubtful : ""),
...
function nwOnIncomeLinkChange(assetId) {
  ...
  if (assetId) {
    if (assetId.startsWith("r")) {
      ...
      if (bizAsset) {
        f.monthlyAmount = String(+bizAsset.monthlyInterestIncome || 0);
...
function nwCommit(){
  ...
  if(f.section === "liabilities"){
    ...
    Object.assign(rowData, {
      loanBookOutstanding: parseFloat(f.loanBookOutstanding)||0,
      monthlyInterestIncome: parseFloat(f.monthlyInterestIncome)||0,
      avgInterestRate: parseFloat(f.avgInterestRate)||0,
      provisionForDoubtful: parseFloat(f.provisionForDoubtful)||0
    });
    ...
    <div style="margin-top:16px;padding:16px;background:var(--bg-input);border-radius:12px;border:1px solid var(--border)">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
        ...
        <div>
          ...
          <input class="num" type="text" inputmode="decimal" value="${fBiz.loanBookOutstanding||""}" oninput="if(/^\d*\.?\d*$/.test(this.value)){STATE.nw._form.loanBookOutstanding=this.value;}else{this.value=STATE.nw._form.loanBookOutstanding||'';}" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div>
          ...
          <input class="num" type="text" inputmode="decimal" value="${fBiz.monthlyInterestIncome||""}" oninput="if(/^\d*\.?\d*$/.test(this.value)){STATE.nw._form.monthlyInterestIncome=this.value;}else{this.value=STATE.nw._form.monthlyInterestIncome||'';}" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div>
          ...
          <input class="num" type="text" inputmode="decimal" value="${fBiz.avgInterestRate||""}" oninput="if(/^\d*\.?\d*$/.test(this.value)){STATE.nw._form.avgInterestRate=this.value;}else{this.value=STATE.nw._form.avgInterestRate||'';}" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div>
          ...
          <input class="num" type="text" inputmode="decimal" value="${fBiz.provisionForDoubtful||""}" oninput="if(/^\d*\.?\d*$/.test(this.value)){STATE.nw._form.provisionForDoubtful=this.value;}else{this.value=STATE.nw._form.provisionForDoubtful||'';}" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
      </div>
  ...
  const resolvedStreams = incomeStreams.map(x => {
    ...
    if (x.linkedAssetId) {
      if (x.linkedAssetId.startsWith("r")) {
        ...
        if (bizAsset) {
          amt = +bizAsset.monthlyInterestIncome || 0;
          own = bizAsset.owner;
          linkLabel = ` (ดึงค่าจากธุรกิจ: ${bizAsset.name})`;
        }
      }
    }
