...
function tabNetWorth(m){
  ...
  const dcaAlert = currentDCA > r.dcaCapacity 
    ? `<div style="display:flex;align-items:center;gap:8px;margin-top:12px;padding:10px 12px;background:rgba(239,68,68,0.15);border:1px solid var(--accent-red);border-radius:8px;color:var(--accent-red);font-size:12px;font-weight:600">
        ...
        <span>DCA ปัจจุบัน (${baht(currentDCA)}) เกินกระแสเงินสดปลอดภัย (Capacity: ${baht(r.dcaCapacity)})</span>
       </div>`
    : `<div style="display:flex;align-items:center;gap:8px;margin-top:12px;padding:10px 12px;background:rgba(16,185,129,0.1);border:1px solid var(--green);border-radius:8px;color:var(--green);font-size:12px;font-weight:600">
        ...
        <span>แผน DCA อยู่ในเกณฑ์ปลอดภัย (${baht(currentDCA)} / Capacity: ${baht(r.dcaCapacity)})</span>
       </div>`;
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4, 1fr);gap:10px;margin-bottom:14px;font-size:12px;text-align:center" class="muted">
        <div style="background:var(--bg-input);padding:10px;border-radius:8px">
          ...
          <div style="font-weight:700;color:var(--text-primary);font-size:13px;margin-top:2px">${baht(r.income)}</div>
        </div>
        <div style="background:var(--bg-input);padding:10px;border-radius:8px">
          <div>รายจ่าย/เดือน</div>
          <div style="font-weight:700;color:var(--text-primary);font-size:13px;margin-top:2px">${baht(r.expense)}</div>
        </div>
        <div style="background:var(--bg-input);padding:10px;border-radius:8px">
          <div>หนี้สิน/เดือน</div>
          <div style="font-weight:700;color:var(--text-primary);font-size:13px;margin-top:2px">${baht(r.debtPayment)}</div>
        </div>
        <div style="background:var(--bg-input);padding:10px;border-radius:8px">
          <div>เบี้ยประกัน/เดือน</div>
          <div style="font-weight:700;color:var(--text-primary);font-size:13px;margin-top:2px">${baht(r.insurancePrem)}</div>
        </div>
      </div>
      ...
      <div style="display:flex;justify-content:space-between;padding:12px 0;border-top:1px solid var(--line);font-size:14px;font-weight:700">
        ...
        <span class="tnum ${r.dcaCapacity >= 0 ? 'green' : 'red'}">${baht(r.dcaCapacity)}</span>
      </div>
  ...
  const emPct = Math.min(100, (r.emergencyMonths / 6) * 100);
  const passivePct = Math.min(100, r.passiveRatio * 100);
  const d2a = r.assets > 0 ? (r.liabilities / r.assets) * 100 : 0;
  const d2aPct = Math.min(100, d2a);

  let emStatusText = "";
  if (r.emergencyMonths < 3) emStatusText = `<span style="color:var(--accent-red)">⚠ ต่ำ (ควรสำรองเพิ่ม)</span>`;
  else if (r.emergencyMonths > 12) emStatusText = `<span style="color:var(--accent-gold)">💡 เงินสดเยอะเกินเกณฑ์ (แนะนำลงทุนเพิ่ม)</span>`;
  else emStatusText = `<span style="color:var(--green)">✅ เพียงพอสำหรับกรณีฉุกเฉิน</span>`;

  let gapText = "";
  if (r.protectionGap > 0) {
    gapText = `<span class="red" style="font-weight:700">${baht(r.protectionGap)} (⚠ คุ้มครองไม่พอหนี้+ครอบครัว)</span>`;
  } else {
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;font-size:12px">
          <span><b>เงินสำรองฉุกเฉิน:</b> <span class="tnum" style="font-weight:700;color:var(--text-primary)">${r.emergencyMonths.toFixed(1)} เดือน</span> (${baht(r.liquidAssets)} / รายจ่ายประจำ ${baht(r.expense + r.debtPayment + r.insurancePrem)}/ด.)</span>
        ...
        <div style="width:100%;height:8px;background:var(--line);border-radius:4px;overflow:hidden;margin-top:6px">
          <div style="width:${emPct}%;height:100%;background:${r.emergencyMonths < 3 ? 'var(--accent-red)' : r.emergencyMonths > 12 ? 'var(--accent-gold)' : 'var(--green)'};border-radius:4px"></div>
        </div>
      </div>
      ...
      <div style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;font-size:12px">
          <span><b>Passive Income Ratio:</b> <span class="tnum" style="font-weight:700;color:var(--text-primary)">${(r.passiveRatio * 100).toFixed(1)}%</span> (เป้าหมาย 100% FIRE)</span>
          <span class="muted">[ปันผล+ค่าเช่า ${baht(r.passiveIncome)} / รายจ่ายประจำ ${baht(r.expense)}/ด.]</span>
        </div>
        <div style="width:100%;height:8px;background:var(--line);border-radius:4px;overflow:hidden;margin-top:6px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div>
            ...
            <div style="font-size:10px;margin-top:2px" class="muted">
              ความต้องการคุ้มครอง (หนี้สิน + ค่าใช้จ่ายครอบครัว ${STATE.nw.global.config.protectionYears} ปี): ${baht(r.needCoverage)}<br>
              ความคุ้มครองที่มีอยู่ (ทุนประกันชีวิต + สภาพคล่อง): ${baht(r.haveCoverage + r.liquidAssets)}
            </div>
          </div>
