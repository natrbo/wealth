...
function nwToggleForm(section){
  ...
  if(STATE.nw._form&&STATE.nw._form.section===section){ STATE.nw._form=null; }
    ...
    STATE.nw._form={section, mode:"add", name:"", type:"mortgage_lending", owner:"household", loanBookOutstanding:0, monthlyInterestIncome:0, avgInterestRate:0, provisionForDoubtful:0};
  }else if(section==="realEstate"){
    STATE.nw._form={section, mode:"add", name:"", type:"rental_condo", owner:"household", marketValue:0, monthlyRentIncome:0, monthlyRentExpense:0, linkedLiabilityId:""};
  }else if(section==="incomeStreams"){
    STATE.nw._form={section, mode:"add", source:"", type:"salary", owner:"household", monthlyAmount:0, passive:false, linkedAssetId:""};
  }
  renderTab();
}
...
function nwEditRow(section, id){
  ...
  STATE.nw._form={
    ...
    liquid: !!row.liquid,
    loanBookOutstanding: row.loanBookOutstanding||0,
    monthlyInterestIncome: row.monthlyInterestIncome||0,
    avgInterestRate: row.avgInterestRate||0,
    provisionForDoubtful: row.provisionForDoubtful||0,
    marketValue: row.marketValue||0,
    monthlyRentIncome: row.monthlyRentIncome||0,
    monthlyRentExpense: row.monthlyRentExpense||0,
...
function nwCommit(){
  ...
  const nameVal = f.section === "incomeStreams" ? f.source.trim() : f.name.trim();
  if(!nameVal){ toast(f.section === "incomeStreams" ? "ใส่แหล่งที่มาก่อน" : "ใส่ชื่อก่อน","var(--red)"); return; }
  ...
  let rowData = {
    ...
    type: f.type,
    owner: f.owner,
    splitFern: sp.splitFern,
    splitNut: sp.splitNut
  };

  if(f.section === "liabilities"){
    Object.assign(rowData, {
      principalRemaining: +f.principalRemaining||0,
    ...
    Object.assign(rowData, {
      loanBookOutstanding: +f.loanBookOutstanding||0,
      monthlyInterestIncome: +f.monthlyInterestIncome||0,
      avgInterestRate: +f.avgInterestRate||0,
      provisionForDoubtful: +f.provisionForDoubtful||0
    });
  } else if(f.section === "realEstate"){
    Object.assign(rowData, {
      marketValue: +f.marketValue||0,
      monthlyRentIncome: +f.monthlyRentIncome||0,
      monthlyRentExpense: +f.monthlyRentExpense||0,
...
function nwDelete(section,id){
  const row=STATE.nw.global[section].find(r=>r.id===id); if(!row) return;
  const dispName = row.name || row.source || "";
...
function tabNetWorth(m){
  ...
  // ── 6. CRUD Section: Manage Business Assets ──
  ...
  const typeOptsBiz=[["mortgage_lending","ปล่อยกู้จำนอง/ขายฝาก"]];
  ...
  const bizForm = fBiz ? `
    <div style="margin-top:16px;padding:16px;background:var(--bg-input);border-radius:12px;border:1px solid var(--border)">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
        <div style="grid-column:1/-1">
          <label style="display:block;margin-bottom:4px;font-size:11px;font-weight:500;color:var(--text-secondary)">ชื่อธุรกิจ / ทรัพย์สิน</label>
        ...
        <div>
          ...
          <select onchange="STATE.nw._form.type=this.value" style="width:100%;padding:8px;font-size:13px;border-radius:8px;background:var(--bg-input);color:var(--text-primary);border:1px solid var(--border)">
            ${typeOptsBiz.map(([v,l])=>`<option value="${v}" ${fBiz.type===v?"selected":""}>${l}</option>`).join("")}
          </select>
        </div>
        <div>
          ...
          <select onchange="STATE.nw._form.owner=this.value" style="width:100%;padding:8px;font-size:13px;border-radius:8px;background:var(--bg-input);color:var(--text-primary);border:1px solid var(--border)">
            ${[["household","ถือร่วม 50/50"],["fern","เฟิร์น"],["nut","นัท"]].map(([v,l])=>`<option value="${v}" ${fBiz.owner===v?"selected":""}>${l}</option>`).join("")}
          </select>
        </div>
        <div>
          <label style="display:block;margin-bottom:4px;font-size:11px;font-weight:500;color:var(--text-secondary)">เงินต้นคงค้างรวม (฿)</label>
          <input class="num" type="text" inputmode="numeric" value="${fBiz.loanBookOutstanding||""}" oninput="STATE.nw._form.loanBookOutstanding=num(this.value)" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div>
          <label style="display:block;margin-bottom:4px;font-size:11px;font-weight:500;color:var(--text-secondary)">ดอกเบี้ยรับเฉลี่ย/เดือน (฿)</label>
          <input class="num" type="text" inputmode="numeric" value="${fBiz.monthlyInterestIncome||""}" oninput="STATE.nw._form.monthlyInterestIncome=num(this.value)" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div>
          <label style="display:block;margin-bottom:4px;font-size:11px;font-weight:500;color:var(--text-secondary)">อัตราดอกเบี้ยเฉลี่ย (% ต่อปี)</label>
          <input class="num" type="text" inputmode="numeric" value="${fBiz.avgInterestRate||""}" oninput="STATE.nw._form.avgInterestRate=num(this.value)" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div>
          <label style="display:block;margin-bottom:4px;font-size:11px;font-weight:500;color:var(--text-secondary)">เงินสำรองหนี้สงสัยจะสูญ (฿)</label>
          <input class="num" type="text" inputmode="numeric" value="${fBiz.provisionForDoubtful||""}" oninput="STATE.nw._form.provisionForDoubtful=num(this.value)" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
      </div>
      <div style="display:flex;gap:10px">
        <button class="btn" onclick="nwCommit()" style="flex:1;background:linear-gradient(135deg,var(--accent-gold2),var(--accent-gold));color:#09090b;font-weight:600;padding:10px;border-radius:8px">${fBiz.mode==="edit"?"บันทึกการแก้ไข ✓":"เพิ่มธุรกิจ ✓"}</button>
  ...
  const bizManageList = businessAssets.length
    ? businessAssets.map(x=>{
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:14px 0;border-top:1px solid var(--line);font-size:13px">
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--text-primary);font-size:14px">${x.name} <span class="muted" style="font-size:10px">(${x.owner==="household"?"ถือร่วม":x.owner==="fern"?"เฟิร์น":"นัท"})</span></div>
            <div class="muted" style="font-size:11px;margin-top:4px">
              ยอดปล่อยกู้: <span style="font-weight:600;color:var(--text-primary)">${baht(x.loanBookOutstanding)}</span> · ดอกเบี้ยรับ: ${baht(x.monthlyInterestIncome)}/ด. (${x.avgInterestRate}%) · หนี้สูญสำรอง: ${baht(x.provisionForDoubtful)}
            </div>
          </div>
          <div style="display:flex;gap:14px;align-items:center">
            <button class="btn" style="background:none;border:none;color:var(--accent-gold);cursor:pointer;font-size:12px;font-weight:700;padding:4px" onclick="nwEditRow('businessAssets','${x.id}')">แก้ไข</button>
            <button class="btn" style="background:none;border:none;color:var(--accent-red);cursor:pointer;font-size:12px;font-weight:700;padding:4px" onclick="nwDelete('businessAssets','${x.id}')">ลบ</button>
          </div>
        </div>`;
      }).join("")
    : `<div class="muted" style="font-size:12px;padding:8px 0">ยังไม่มีรายการธุรกิจปล่อยจำนอง</div>`;

  const crudBizCard=`<div class="card" style="margin-bottom:20px;padding:20px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
      <div class="ctitle" style="margin-bottom:0">จัดการธุรกิจปล่อยจำนอง</div>
      <button class="btn" style="background:var(--ink3);color:var(--gold);border:1px solid var(--line);padding:6px 14px;font-size:12px;border-radius:8px" onclick="nwToggleForm('businessAssets')">${fBiz?"✕ ปิด":"＋ เพิ่มธุรกิจ"}</button>
    </div>
    ${bizManageList}
    ${bizForm}
  </div>`;
    ...
    <div style="margin-top:16px;padding:16px;background:var(--bg-input);border-radius:12px;border:1px solid var(--border)">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
        ...
        <div>
          ...
          <input class="num" type="text" inputmode="numeric" value="${fRe.marketValue||""}" oninput="STATE.nw._form.marketValue=num(this.value)" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div>
          ...
          <input class="num" type="text" inputmode="numeric" value="${fRe.monthlyRentIncome||""}" oninput="STATE.nw._form.monthlyRentIncome=num(this.value)" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div>
          ...
          <input class="num" type="text" inputmode="numeric" value="${fRe.monthlyRentExpense||""}" oninput="STATE.nw._form.monthlyRentExpense=num(this.value)" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div style="grid-column:1/-1">
  ...
  const incForm = fInc ? `
    <div style="margin-top:16px;padding:16px;background:var(--bg-input);border-radius:12px;border:1px solid var(--border)">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
        ...
        <div>
          <label style="display:block;margin-bottom:4px;font-size:11px;font-weight:500;color:var(--text-secondary)">ยอดเงินสุทธิ/เดือน (฿)</label>
          <input class="num" type="text" inputmode="numeric" value="${fInc.monthlyAmount||""}" oninput="STATE.nw._form.monthlyAmount=num(this.value)" style="width:100%;padding:8px 12px;font-size:14px;text-align:right;border-radius:8px">
        </div>
        <div style="display:flex;align-items:flex-end;padding-bottom:6px">
          <label style="font-size:11px;display:flex;align-items:center;gap:6px;cursor:pointer">
        ...
        <div style="grid-column:1/-1">
          ...
          <select onchange="STATE.nw._form.linkedAssetId=this.value" style="width:100%;padding:8px;font-size:13px;border-radius:8px;background:var(--bg-input);color:var(--text-primary);border:1px solid var(--border)">
  ...
  const incManageList = incomeStreams.length
    ? incomeStreams.map(x=>{
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:14px 0;border-top:1px solid var(--line);font-size:13px">
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--text-primary);font-size:14px">${x.source} <span class="muted" style="font-size:10px">(${x.owner==="household"?"ถือร่วม":x.owner==="fern"?"เฟิร์น":"นัท"})</span></div>
            <div class="muted" style="font-size:11px;margin-top:4px">
  ...
  m.innerHTML=head+assetsCard+liabSummaryCard+incomeCard+crudAssetsCard+crudLiabCard+crudBizCard+crudReCard+crudIncCard;
}
