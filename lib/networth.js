...
function tabNetWorth(m){
  ...
  const metricsAgg=metricsAll(nwCtx()), rm=metricsAgg[view];
  ...
  const bizItems = businessAssets.map(x => {
    const share = view === "household" ? 1 : splitFor(x, view);
    const loans = loanBook.filter(l => l.businessId === x.id);
    const loansSum = loans.reduce((sum, l) => sum + (+l.principalOutstanding || 0), 0);
    const equity = loansSum + (+x.additionalCapital || 0);
    const netVal = equity * ((+x.ownershipPct || 0) / 100) * share;
    return `<div style="display:flex;justify-content:space-between;padding:4px 0 4px 12px;font-size:12px;color:var(--text-secondary)">
      <span>• ${x.name} (สัดส่วน ${x.ownershipPct}%, ทุน ${baht(x.additionalCapital * share)})</span>
  ...
  // --- Calculate current DCA plans to show alerts ---
  const fernDCA = (STATE.nw.fern || []).reduce((s, a) => s + (+a.plan || 0), 0);
  const nutDCA = (STATE.nw.nut || []).reduce((s, a) => s + (+a.plan || 0), 0);
  const currentDCA = view === "household" ? (fernDCA + nutDCA) : (view === "fern" ? fernDCA : nutDCA);

  const dcaAlert = currentDCA > rm.safeDCACapacity 
    ? `<div style="display:flex;align-items:center;gap:8px;margin-top:12px;padding:10px 12px;background:rgba(239,68,68,0.15);border:1px solid var(--accent-red);border-radius:8px;color:var(--accent-red);font-size:12px;font-weight:600">
        ...
        <span>DCA ปัจจุบัน (${baht(currentDCA)}) เกินกระแสเงินสดปลอดภัย (Capacity: ${baht(rm.safeDCACapacity)})</span>
       </div>`
    : `<div style="display:flex;align-items:center;gap:8px;margin-top:12px;padding:10px 12px;background:rgba(16,185,129,0.1);border:1px solid var(--green);border-radius:8px;color:var(--green);font-size:12px;font-weight:600">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
        <span>แผน DCA อยู่ในเกณฑ์ปลอดภัย (${baht(currentDCA)} / Capacity: ${baht(rm.safeDCACapacity)})</span>
       </div>`;

  const cashFlowDetailsCard = `
    <div class="card" style="margin-bottom:16px">
      <div class="ctitle">กระแสเงินสดรายเดือน (Cash Flow & DCA Capacity)</div>
      
      <div style="display:grid;grid-template-columns:repeat(4, 1fr);gap:10px;margin-bottom:14px;font-size:12px;text-align:center" class="muted">
        <div style="background:var(--bg-input);padding:10px;border-radius:8px">
          <div>รายได้/เดือน</div>
          <div style="font-weight:700;color:var(--text-primary);font-size:13px;margin-top:2px">${baht(rm.income)}</div>
        </div>
        <div style="background:var(--bg-input);padding:10px;border-radius:8px">
          <div>รายจ่าย/เดือน</div>
          <div style="font-weight:700;color:var(--text-primary);font-size:13px;margin-top:2px">${baht(rm.expense)}</div>
        </div>
        <div style="background:var(--bg-input);padding:10px;border-radius:8px">
          <div>หนี้สิน/เดือน</div>
          <div style="font-weight:700;color:var(--text-primary);font-size:13px;margin-top:2px">${baht(rm.debtPayment)}</div>
        </div>
        <div style="background:var(--bg-input);padding:10px;border-radius:8px">
          <div>เบี้ยประกัน/เดือน</div>
          <div style="font-weight:700;color:var(--text-primary);font-size:13px;margin-top:2px">${baht(rm.insurancePrem)}</div>
        </div>
      </div>

      <div style="display:flex;justify-content:space-between;padding:12px 0;border-top:1px solid var(--line);font-size:14px;font-weight:700">
        <span>กระแสเงินสดปลอดภัย (DCA Capacity)</span>
        <span class="tnum ${rm.safeDCACapacity >= 0 ? 'green' : 'red'}">${baht(rm.safeDCACapacity)}</span>
      </div>
  ...
  const emPct = rm.emergencyMonths === Infinity ? 100 : Math.min(100, (rm.emergencyMonths / 6) * 100);
  const passivePct = Math.min(100, rm.passiveRatio * 100);
  const d2a = r.assets > 0 ? (r.liabilities / r.assets) * 100 : 0;
  const d2aPct = Math.min(100, d2a);

  let emStatusText = "";
  if (rm.emergencyMonths < 3) emStatusText = `<span style="color:var(--accent-red)">⚠ ต่ำ (ควรสำรองเพิ่ม)</span>`;
  else if (rm.emergencyMonths > 12) emStatusText = `<span style="color:var(--accent-gold)">💡 เงินสดเยอะเกินเกณฑ์ (แนะนำลงทุนเพิ่ม)</span>`;
  else emStatusText = `<span style="color:var(--green)">✅ เพียงพอสำหรับกรณีฉุกเฉิน</span>`;

  let gapText = "";
  if (rm.protectionGap > 0) {
    gapText = `<span class="red" style="font-weight:700">${baht(rm.protectionGap)} (⚠ คุ้มครองไม่พอหนี้+ครอบครัว)</span>`;
  } else {
    gapText = `<span class="green" style="font-weight:700">✅ เพียงพอ (ครอบคลุมความต้องการ)</span>`;
  }

  const emValText = rm.emergencyMonths === Infinity ? "∞" : rm.emergencyMonths.toFixed(1);

  const keyMetricsCard = `
    <div class="card" style="margin-bottom:16px">
      <div class="ctitle">ตัวชี้วัดทางการเงินที่สำคัญ (Key Financial Metrics)</div>
      
      <!-- Emergency Fund -->
      <div style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;font-size:12px">
          <span><b>เงินสำรองฉุกเฉิน:</b> <span class="tnum" style="font-weight:700;color:var(--text-primary)">${emValText} เดือน</span> (${baht(rm.liquidAssets)} / รายจ่ายประจำ ${baht(rm.expense + rm.debtPayment + rm.insurancePrem)}/ด.)</span>
          <span>${emStatusText}</span>
        </div>
        <div style="width:100%;height:8px;background:var(--line);border-radius:4px;overflow:hidden;margin-top:6px">
          <div style="width:${emPct}%;height:100%;background:${rm.emergencyMonths < 3 ? 'var(--accent-red)' : rm.emergencyMonths > 12 ? 'var(--accent-gold)' : 'var(--green)'};border-radius:4px"></div>
        </div>
      </div>
      ...
      <div style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;font-size:12px">
          <span><b>Passive Income Ratio:</b> <span class="tnum" style="font-weight:700;color:var(--text-primary)">${(rm.passiveRatio * 100).toFixed(1)}%</span> (เป้าหมาย 100% FIRE)</span>
          <span class="muted">[ปันผล+ค่าเช่า ${baht(r.passiveIncome)} / รายจ่ายประจำ ${baht(rm.expense)}/ด.]</span>
        </div>
        <div style="width:100%;height:8px;background:var(--line);border-radius:4px;overflow:hidden;margin-top:6px">
          <div style="width:${passivePct}%;height:100%;background:var(--green);border-radius:4px"></div>
        </div>
      </div>
      ...
      <div style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;font-size:12px">
          <span><b>อัตราส่วนหนี้สินต่อสินทรัพย์ (Debt-to-Asset):</b> <span class="tnum" style="font-weight:700;color:var(--text-primary)">${d2a.toFixed(1)}%</span> (แนะนำ &lt; 50%)</span>
          <span class="${d2a > 50 ? 'red' : 'green'}">${d2a > 50 ? '⚠ สูงเกินเกณฑ์' : '✅ ปลอดภัย'}</span>
        </div>
        <div style="width:100%;height:8px;background:var(--line);border-radius:4px;overflow:hidden;margin-top:6px">
          <div style="width:${d2aPct}%;height:100%;background:${d2a > 50 ? 'var(--accent-red)' : 'var(--green)'};border-radius:4px"></div>
        </div>
      </div>

      <!-- Protection Gap -->
      <div style="padding-top:10px;border-top:1px solid var(--line);font-size:12px">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div>
            <b>ช่องว่างความคุ้มครองชีวิต (Protection Gap):</b>
            <div style="font-size:10px;margin-top:2px" class="muted">
              ความต้องการคุ้มครอง (หนี้สิน + ค่าใช้จ่ายครอบครัว ${STATE.nw.global.config.protectionYears} ปี): ${baht(rm.needCoverage)}<br>
              ความคุ้มครองที่มีอยู่ (ทุนประกันชีวิต + สภาพคล่อง): ${baht(rm.haveCoverage + rm.liquidAssets)}
            </div>
          </div>
          <div style="text-align:right">${gapText}</div>
        </div>
      </div>
    </div>
  `;

  m.innerHTML=head+assetsCard+liabSummaryCard+incomeCard+cashFlowDetailsCard+keyMetricsCard+crudAssetsCard+crudLiabCard+crudReCard+crudIncCard+crudCashFlowCard+crudInsuranceCard;
  } catch(err) {
    console.error(err);
    m.innerHTML=`<div class="card" style="border:1px solid var(--accent-red)"><div class="red" style="font-weight:bold;margin-bottom:8px">เกิดข้อผิดพลาดในการวาดหน้าจอ (Render Error)</div><pre style="background:var(--bg-input);padding:12px;border-radius:8px;font-size:11px;overflow-x:auto;color:var(--accent-red);user-select:text">${err.stack||err}</pre></div>`;
  }
}
...
function tabBusiness(m){
  ...
  `;
    ...
    console.error(err);
    m.innerHTML=`<div class="card" style="border:1px solid var(--accent-red)"><div class="red" style="font-weight:bold;margin-bottom:8px">เกิดข้อผิดพลาดในการวาดหน้าจอธุรกิจ (Render Error)</div><pre style="background:var(--bg-input);padding:12px;border-radius:8px;font-size:11px;overflow-x:auto;color:var(--accent-red);user-select:text">${err.stack||err}</pre></div>`;
  }
}
