...
(function (root, factory) {
  ...
  // ctx = { portfolios:{fern:[],nut:[]}, liabilities:[], assetsOther:[], businessAssets:[], realEstate:[], incomeStreams:[] }
  function netWorthParts(p, ctx) {
    ...
    const biz = (ctx.businessAssets || []).reduce((s, r) => s + Math.max(0, (+r.loanBookOutstanding || 0) - (+r.provisionForDoubtful || 0)) * splitFor(r, p), 0);
    ...
    const passiveIncome = (ctx.incomeStreams || []).filter(r => r.passive === true || r.passive === "true" || r.passive === "TRUE" || r.passive === 1)
      .reduce((s, r) => s + (+r.monthlyAmount || 0) * splitFor(r, p), 0);
    const activeIncome = (ctx.incomeStreams || []).filter(r => !(r.passive === true || r.passive === "true" || r.passive === "TRUE" || r.passive === 1))
      .reduce((s, r) => s + (+r.monthlyAmount || 0) * splitFor(r, p), 0);
