...
(function (root, factory) {
  ...
  function financialMetrics(p, ctx) {
    ctx = ctx || {};
    const nw = netWorthParts(p, ctx);
    ...
    const income = nw.activeIncome + nw.passiveIncome;
    const currentDCA = ((ctx.portfolios || {})[p] || []).reduce((s, a) => s + (+a.plan || 0), 0);
    ...
    const safeDCACapacity = income - expense - debtPayment - insurancePrem;
    const dcaOverflow = currentDCA > safeDCACapacity;
    const emergencyMonths = monthlyBurn > 0 ? liquidAssets / monthlyBurn : Infinity;
    const passiveRatio = expense > 0 ? nw.passiveIncome / expense : 0;
    
    const config = ctx.config || { expectedReturn: 7, protectionYears: 10 };
    const protectionYears = +config.protectionYears || 10;
    const needCoverage = nw.liabilities + (expense * 12 * protectionYears);
    
    const haveCoverage = (ctx.insurance || []).filter(r => r.insured === p && (r.type === "life" || r.type === "LIFE"))
      .reduce((s, r) => s + (+r.sumAssured || 0), 0);
      
    const protectionGap = needCoverage - haveCoverage;
  ...
  function metricsAll(ctx) {
    const fern = financialMetrics("fern", ctx);
    const nut = financialMetrics("nut", ctx);
    
    const hhExpense = fern.expense + nut.expense;
    const hhDebtPayment = fern.debtPayment + nut.debtPayment;
    const hhInsurancePrem = fern.insurancePrem + nut.insurancePrem;
    const hhMonthlyBurn = hhExpense + hhDebtPayment + hhInsurancePrem;
    const hhIncome = fern.income + nut.income;
    const hhCurrentDCA = fern.currentDCA + nut.currentDCA;
    const hhLiquidAssets = fern.liquidAssets + nut.liquidAssets;
    
    const nwFern = netWorthParts("fern", ctx);
    const nwNut = netWorthParts("nut", ctx);
    const hhLiabilities = nwFern.liabilities + nwNut.liabilities;
    
    const safeDCACapacity = hhIncome - hhExpense - hhDebtPayment - hhInsurancePrem;
    
    const config = ctx.config || { expectedReturn: 7, protectionYears: 10 };
    const protectionYears = +config.protectionYears || 10;
    const hhNeedCoverage = hhLiabilities + (hhExpense * 12 * protectionYears);
    const hhHaveCoverage = fern.haveCoverage + nut.haveCoverage;
    
    const household = {
      expense: hhExpense,
      debtPayment: hhDebtPayment,
      insurancePrem: hhInsurancePrem,
      income: hhIncome,
      currentDCA: hhCurrentDCA,
      liquidAssets: hhLiquidAssets,
      monthlyBurn: hhMonthlyBurn,
      safeDCACapacity,
      dcaOverflow: hhCurrentDCA > safeDCACapacity,
      emergencyMonths: hhMonthlyBurn > 0 ? hhLiquidAssets / hhMonthlyBurn : Infinity,
      passiveRatio: hhExpense > 0 ? (nwFern.passiveIncome + nwNut.passiveIncome) / hhExpense : 0,
      needCoverage: hhNeedCoverage,
      haveCoverage: hhHaveCoverage,
      protectionGap: hhNeedCoverage - hhHaveCoverage
    };
